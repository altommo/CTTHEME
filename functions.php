...
// AJAX handler for getting liked videos
function customtube_get_liked_videos() {
    ...
    // Get liked video IDs for current user
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            ...
            $videos[] = array(
                ...
                'liked' => true // All videos in this list are liked
            );
        }
    }
...
/**
 * CSS Cache Busting
 * Automatically increment version when CSS files are modified
 */
function customtube_get_css_version() {
    // Point to the compiled main.css file
    ...
    return CUSTOMTUBE_VERSION; // Fallback to theme version
}

/**
 * JavaScript Cache Busting
 * Automatically increment version when JS files are modified
 */
function customtube_get_js_version() {
    // Point to the compiled main JavaScript file
...
function customtube_get_page_template($page_template) {
    // This function is now largely redundant because `theme_page_templates` filter
    // already maps the correct path. However, keeping it for explicit control
    // or if there are specific overrides needed.
    
    $template_slug = get_page_template_slug();
    if (empty($template_slug)) {
        return $page_template; // No custom template assigned
    }
...
if (!function_exists('customtube_get_trending_videos')) {
    function customtube_get_trending_videos($count = 8) {
        ...
        if (false === $query) {
            ...
            $query = new WP_Query($args);
            set_transient($cache_key, $query, HOUR_IN_SECONDS); // Cache for 1 hour
        }
        return $query;
    }
}
...
if (!function_exists('customtube_get_recent_videos')) {
    function customtube_get_recent_videos($count = 8) {
        ...
        if (false === $query) {
            ...
            $query = new WP_Query($args);
            set_transient($cache_key, $query, DAY_IN_SECONDS); // Cache for 1 day
        }
        return $query;
    }
}
 ...
 *
 * @param string $duration Duration term slug (short, medium, long)
 * @param int $count Number of videos to retrieve
 * @return WP_Query
 */
if (!function_exists('customtube_get_videos_by_duration')) {
    function customtube_get_videos_by_duration($duration = 'medium', $count = 8) {
        ...
        if (false === $query) {
            ...
            // Add meta query if duration is specified
            if (!empty($meta_query)) {
 ...
 *
 * @param int $count Number of categories to retrieve
...
if (!function_exists('customtube_get_popular_categories')) {
    function customtube_get_popular_categories($count = 8) {
        ...
        if (false === $categories) {
            // Try to get genre taxonomy first (which appears to be the main category taxonomy)
            $categories = get_terms(array(
                'taxonomy'   => 'genre',
                'hide_empty' => true, // Corrected from 't'
                'number'     => $count,
                'orderby'    => 'count',
                'order'      => 'DESC',
            ));

            // If no categories or error, try standard category
            ...
            set_transient($cache_key, $categories, DAY_IN_SECONDS); // Cache for 1 day
        }
        return $categories;
    }
}
